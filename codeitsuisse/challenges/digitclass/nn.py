...
class NN(object):
	...
	def load_weight(self,path):
		...
		try:
			self.weights = np.load(os.path.join(path,'weights.npy'),encoding="latin1").item()
		except:
			raise Exception('Not able to load Network weights')

		try:
			self.biases = np.load(os.path.join(path,'biases.npy'),encoding="latin1").item()
		except:
			raise Exception('Not able to load Network biases')

		try:
			self.batch_norm_params = np.load(os.path.join(path,'bn_params.npy'),encoding="latin1").item()
		except:
			raise Exception('Not able to load Network weights')
